---
name: praxis-capture-codebase-patterns
...
Exemplar: the arguments supplied with this skill — if empty, propose candidates in step 2 rather than
picking silently.
...
2. **Exemplar interview.** Ask the user to point at one to three modules or
   ...
   - Recently touched: run `git log --oneline -20 2>/dev/null`
   - Frequently touched: run `git log --pretty=format: --name-only -50 2>/dev/null | sort | uniq -c | sort -rn | head -20`
