...
- Timestamp: run `date +%Y-%m-%d_%H%M`

Build the filename `YYYY-MM-DD_HHMM_HANDOFF_<topic>.md`:
- `<topic>` is a short kebab-case slug for the work. Use the arguments supplied with this skill if
...
- Status: run `git status --short 2>/dev/null`
- Recent commits: run `git log --oneline -10 2>/dev/null`
