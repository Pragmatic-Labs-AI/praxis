...
Focus: the arguments supplied with this skill — if empty, refresh and lint the whole wiki at a high level.
...
- Existing entries: run `ls -R docs/wiki wiki .praxis/wiki 2>/dev/null || true`
- Today (for log entries, do not guess): run `date +%Y-%m-%d`
